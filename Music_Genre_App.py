...
def load_and_preprocess_data(file_path, target_shape=(150, 150)):
    ...
    for i in range(num_chunks):
        ...
        if len(chunk) < chunk_samples:  # Pad if chunk is shorter
...
def model_prediction(X_test):
    ...
    st.write("Raw Predictions:", y_pred)  # Debugging: Log raw predictions
    predicted_categories = np.argmax(y_pred, axis=1)
    unique_elements, counts = np.unique(predicted_categories, return_counts=True)
    st.write("Predicted Categories and Counts:", unique_elements, counts)  # Debugging: Log category counts
...
if(app_mode=="Home"):
    ...
    st.image(image_path, use_container_width=True)  # Updated line
    st.markdown("""
...
#Prediction Page
    ...
    if test_mp3 is not None:
        ...
        with open(filepath, "wb") as f:
            f.write(test_mp3.getbuffer())  # Save uploaded file locally
    ...
    if(st.button("Predict")):
        with st.spinner("Please Wait.."):
            try:
                ...
                st.write("Preprocessed Data Shape:", X_test.shape)  # Debugging: Log data shape
                result_index = model_prediction(X_test)
                st.balloons()
                label = ['blues', 'classical', 'country', 'disco', 'hiphop', 'jazz', 'metal', 'pop', 'reggae', 'rock']
                st.markdown("**:blue[Model Prediction:] It's a  :red[{}] music**".format(label[result_index]))
            except Exception as e:
                st.error(f"Error during prediction: {e}")  # Handle errors gracefully
